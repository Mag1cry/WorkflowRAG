...
def build_injection(task_key: str, checkpointer, db_path: Path,
                    embedding: M3EEmbedding, pruner: str = "llm",
                    judge_llm=None) -> str | None:
    """从 run 的 checkpointer 提取 → 剪枝（规则或 LLM）→ 固化 → 检索 → 生成注入文本。

    Returns:
        (injection_text, judge_stats | None)
    """
    spec = TASKS_REGISTRY[task_key]
    ...
    try:
        ...
        pruner_tag = "LLM" if pruner == "llm" else "规则"
        print(f"  [注入] {wf_id} | RAW {raw_count} → SOLIDIFIED {kept_count} "
              f"({pruner_tag}剪枝) | 注入 {len(injection)} chars")
...
def run_mode(task_key: str, mode: str, runs: int, db_dir: Path, pruner: str = "llm") -> dict:
    spec = TASKS_REGISTRY[task_key]
    results: dict = {"task": task_key, "mode": mode, "pruner": pruner, "runs": []}
    embedding = None
    ...
    if mode == "baseline":
        ...
        db_path = db_dir / f"{task_key}_inject_{pruner}.db"
        if db_path.exists():
            db_path.unlink()
        # run 1: 无注入，产出 workflow
        stats1, cp1 = run_once(task_key, "thread_1")
        print(f"  [inject:{pruner}] run 1 (raw) | calls={stats1['tool_call_count']} "
        ...
        injection, judge_stats = build_injection(
            task_key, cp1, db_path, embedding, pruner=pruner, judge_llm=judge_llm)
        ...
        for i in range(2, runs + 1):
            ...
            print(f"  [inject:{pruner}] run {i}/{runs} (injected) | calls={stats['tool_call_count']} "
                  f"tokens={stats['total_tokens']} passed={stats['passed']}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Workflow 省 Token 端到端评测")
    ...
    parser.add_argument("--pruner", choices=["rule", "llm"], default="llm",
                        help="注入模式的剪枝方式（rule=规则剪枝, llm=LLM 剪枝）")
    ...
    for task_key in tasks:
        for mode in modes:
            if mode == "baseline":
                pruners = ["rule"]  # 占位，baseline 不涉及剪枝
            else:
                pruners = [args.pruner]
            for pruner in pruners:
                tag = "" if mode == "baseline" else f"_{pruner}"
                print(f"\n=== {task_key} [{mode}{tag}] runs={args.runs} ===")
                results = run_mode(task_key, mode, args.runs, db_dir, pruner=pruner)
                out = RESULTS_DIR / f"{task_key}_{mode}{tag}.json"
                out.write_text(json.dumps(results, ensure_ascii=False, indent=2), encoding="utf-8")
                s = summarize(results)
                s["mode_tag"] = f"{mode}{tag}"
                summary_rows.append({"task": task_key, "mode": f"{mode}{tag}", **s})
                print(f"  汇总: {s}")
