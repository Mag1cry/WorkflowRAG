...
def render(data: dict) -> str:
    lines = [
        ...
        "",
        f"_生成时间: {__import__('datetime').datetime.now().isoformat(timespec='seconds')}_",
        "",
        "## 1. 汇总对比",
        "",
    ...
    for task in ("T1", "T2", "T3", "T4"):
        ...
        for mode_tag in sorted(td):
            ...
            lines.append(
                ...
                f"{avg(runs, 'steps')} | {round(ok / len(runs) * 100, 1)}% | {avg(runs, 'elapsed_s')} |"
            )
    ...
    for task in ("T1", "T2", "T3", "T4"):
        ...
        for mode_tag in sorted(td):
            ...
            lines.append(
                f"| {task} [{mode_tag}] | {bt} | {it} | {saving} | {rate}% | {bc} | {ic} | "
                f"{round(bc - ic, 1) if bc and ic else None} | {len(inj_text)} chars | {judge_tokens} |"
            )
    lines += ["", "## 3. 注入内容样本", ""]
    for task in ("T1", "T2", "T3", "T4"):
        for mode_tag in sorted(data.get(task, {})):
            ...
            if inj:
                lines += [f"### {task} [{mode_tag}] {TASKS_CN.get(task, '')}", "",
                          "```", inj, "```", ""]
    lines += ["", "## 4. LLM 剪枝成本明细", ""]
    lines += ["| 任务 | 模式 | 工具调用 | input tokens | output tokens | 总tokens | 轮数 | done | 报告摘要 |",
              "|---|---|---|---|---|---|---|---|"]
    for task in ("T1", "T2", "T3", "T4"):
        for mode_tag in sorted(data.get(task, {})):
            ...
            if judge:
                ...
                lines.append(
                    f"| {task} [{mode_tag}] | {mode_tag} | {len(judge.get('tool_calls', []))} | "
                    f"{judge.get('input_tokens', 0)} | {judge.get('output_tokens', 0)} | "
                    f"{judge.get('total_tokens', 0)} | {judge.get('rounds', 0)} | "
                    f"{judge.get('done', False)} | {report} |"
                )
    lines += ["", "## 5. 判定", "",
              "> 结论待填：净收益 ≥30% 且成功率不降 → 继续；±30% → 调优；负收益 → 转型/收尾。", ""]
    return "\n".join(lines)
